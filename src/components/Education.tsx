...
function EducationCard({ entry, index }: { entry: EducationEntry; index: number }) {
  return (
    <motion.article
      className="rounded-3xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm hover:shadow-lg hover:border-orange-400/60 transition-all duration-300"
    ...
    >
      <div className="flex flex-col sm:flex-row items-stretch">
        {/* Logo & period */}
        <div className="sm:w-56 flex-shrink-0 bg-slate-50 dark:bg-slate-800/50 p-6 flex flex-col items-center justify-center text-center border-b sm:border-b-0 sm:border-r border-slate-200/80 dark:border-slate-800">
          <div className="w-20 h-20 rounded-2xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-center p-2">
          ...
          <span className="mt-4 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-teal-50 dark:bg-teal-950/60 text-teal-800 dark:text-teal-300 border border-teal-200 dark:border-teal-800">
...
export default function Education() {
  return (
    <section id="education" className="section bg-white dark:bg-slate-900/40 text-foreground w-full py-20 border-t border-slate-200/70 dark:border-slate-800/70">
